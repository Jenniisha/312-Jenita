# References
- [Workshops List](https://vw-angular.s3.ap-south-1.amazonaws.com/index.html)
- [Hosting your Node.js + MongoDB app using Heroku + MongoDB Atlas](https://www.one-tab.com/page/RFHx6MIfT1-B0N6ZDH6XvA)
- [Event Emitter / Event Bus in JavaScript - A Design Pattern](https://pierfrancesco-soffritti.medium.com/create-a-simple-event-bus-in-javascript-8aa0370b3969#:~:text=What's%20an%20event%20bus,to%20know%20the%20event%20bus.)

## Node JS

## Official Guide
- [Node.js Official Guide](https://nodejs.dev/learn)

## Official Docs
- [Node.js website](https://nodejs.org/en/)
- [Express website](https://expressjs.com/)
- [Express Generator](https://expressjs.com/en/starter/generator.html)
- [Nodemon site](https://nodemon.io/)
- [EJS templating](http://ejs.co/)
- [Mongoose site](http://mongoosejs.com/)

## Video Tutorials

### Playlists (links can be found below)
- [Node JS Tutorial for Beginners #1 - Introduction - YouTube Channel - The Net Ninja](https://www.youtube.com/watch?v=w-7RQ46RgxU&list=PL4cUxeGkcC9gcy9lrvMJ75z9maRw4byYp)
- [Node.js Basic - YouTube Channel - Java Brains](https://www.youtube.com/playlist?list=PLqq-6Pq4lTTa-d0iZg41U2RDqECol9C5B)
- [Node.js and Express.js - Full Course - YouTube Channel - freeCodeCamp.org](https://www.youtube.com/watch?v=Oe421EPjeBE)

### Short introductions
- [Node.js Tutorial for Beginners: Learn Node in 1 Hour - YouTube Channel - Programming with Mosh](https://www.youtube.com/watch?v=TlB_eWDSMt4&t=94s)
- [Node.js Crash Course - YouTube Channel - Traversy Media](https://www.youtube.com/watch?v=fBNz5xF-Kx4)
- [Express JS Crash Course - YouTube Channel - Traversy Media](https://www.youtube.com/watch?v=L72fhGm1tfE)
- [nodeschool.io](https://nodeschool.io/)

## Miscelleneous Node JS References
- [Intro to npm scripts on Freecodecamp](https://medium.freecodecamp.org/introduction-to-npm-scripts-1dbb2ae01633)
- [Awesome npm resources and tips](https://github.com/sindresorhus/awesome-npm)
- [Download trends of npm packages - Can also be used to compare trends of similar packages](https://www.npmtrends.com/)
- [Most depended npm modules](https://www.npmjs.com/browse/depended)
- [Awesome Node.js packages](https://github.com/sindresorhus/awesome-nodejs)
- [dotenv package for managing environment variables during development](https://www.npmjs.com/package/dotenv)
- [JS feature support in Node.js versions](http://node.green/)
- [Digital Transformation using Node.js - A Forrester Report](https://reprints.forrester.com/#/assets/2/796/RES135952/reports)

### Miscelleneous
- [Find out process occupying a port on Mac OSX system](https://stackoverflow.com/questions/3855127/find-and-kill-process-locking-port-3000-on-mac)
- [Installing Redis and getting started](https://redis.io/topics/quickstart)
- [Learn Redis](https://try.redis.io/)
- [Using Redix with Node.js (node_redis)](https://docs.redislabs.com/latest/rs/references/client_references/client_nodejs/)
